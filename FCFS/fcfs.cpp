 ...
 * and three I/O queues. All the queues adopt the first-come-first-serve policy.
 * 
 * Author: 			Chun-kai Chang
 * Version: 		2
 * Creation Date:	Thr Dec 12 15:00:00 2013
 * Filename:		fcfs.cpp
 * History:
 *		CK		1	Thr Dec 12 15:00:00 2013
 *				First written
 *		CK		2 	Sat Dec	14	10:32:11 2013
...
#define JOB_EXE_TIME_RANGE 1000
#define ARG 5
...
static unsigned long long rq_busy_sum;//sum of time when the queue is busy
static unsigned long long wq_busy_sum;//used to calculate utilization of the queue
...
void init(){
	//Init queues
	n_rq = 0; n_wq = 0; n_dq= 0; n_eq = 0; n_tq = 0;
	//Init statistics
...
void job_arrival(){
	...
	//job.signal = rand()%2;// 0 or 1
	job.exe_time = rand()%JOB_EXE_TIME_RANGE;
...
void rq_enqueue(job_t job){
	...
	//cout << "Job come to rq at "<<cur_time<<endl;
	//if queue length is nonzero and no greater than number of cpu,
	//	schedule next job to leave run queue.
	//NOTE: rq_dequeue() will handle the case for longer queue length.  
	if(n_rq > 0 && n_rq <= NUM_CPU)
		sched_departure(LEAVE_RQ);
	else{
...
void rq_dequeue(){
	...
	//check whether the departure job carrying a signal
	job_t job = rq.top();
	rq.pop();
	...
	//schedule next job leaving run queue
	if(n_rq > NUM_CPU-1)
		sched_departure(LEAVE_RQ);
...
void dq_enqueue(job_t job){
	...
	if(n_dq == 1) sched_departure(LEAVE_DQ);
}
...
void dq_dequeue(){
	...
	n_dq--;
	n_dq_leave++;
	if(n_dq > 0) sched_departure(LEAVE_DQ);
...
void eq_enqueue(job_t job){
	...
	//schedule next job to leave
	if(n_eq == 1) sched_departure(LEAVE_EQ);
}
...
void eq_dequeue(){
	...
	n_eq--;
	n_eq_leave++;
	if(n_eq > 0) sched_departure(LEAVE_EQ);
...
void tq_enqueue(job_t job){
	...
	//schedule next job to leave
	if(n_tq == 1) sched_departure(LEAVE_TQ);
}
...
void tq_dequeue(){
	...
	n_tq--;
	n_tq_leave++;
	if(n_tq > 0) sched_departure(LEAVE_TQ);
...
void sched_departure(int type){
	unsigned long long depart_time;
	event_t evt;
	
	switch(type){
		case LEAVE_RQ: {
			depart_time = cur_time + expo(RQ_MEAN_SERVICE_TIME);
			evt.type = LEAVE_RQ; 
